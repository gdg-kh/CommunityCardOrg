...
4. **MCP 伺服器解耦**：MCP 伺服器在 `2.0.0` 版本中已將路徑解耦為環境變數 `COMMUNITY_CARD_SUB_PATH`（預設 `"2026"`）。你只需要在年底將其改為 `"v2"`，無須修改任何伺服器代碼。
...
`community-card-mcp` 伺服器在 `2.0.0` 版本中，已將所有年份硬編碼移出，並改為讀取環境變數 `COMMUNITY_CARD_SUB_PATH`（預設為 `"2026"`）。
...
> * 2.0.0 版本實作的「動態滑動窗口驗證」（當前時間 -1 個月至 +12 個月）將在 v2 目錄下終身免維護自動運作！
